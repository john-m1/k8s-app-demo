...
import com.metro.atwater.data.luns.LunsDao;
...
public class LunsController {
		...
		LunsDao lunsDao = (LunsDaoImpl) context.getBean("lunsDao"); 
		
		// POST a json body representing a transaction 
		@RequestMapping(value = "/luns", method = RequestMethod.POST, consumes = "application/json")
		@ResponseBody
		public String process(@RequestBody final String body) throws Exception {
			...
			try {
				result = mapper.readValue(body, Luns.class);
		...
		@RequestMapping(value = "/luns", method = RequestMethod.DELETE)
		@ResponseBody
